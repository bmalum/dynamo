defprotocol Dynamo.Encodable do
  @type t :: any
...
defimpl Dynamo.Encodable, for: Atom do
...
defimpl Dynamo.Encodable, for: Integer do
  def encode(val, _) do
    %{"N" => val |> Integer.to_string()}
  end
end

defimpl Dynamo.Encodable, for: Float do
  def encode(val, _) do
    %{"N" => String.Chars.Float.to_string(val)}
  end
end

defimpl Dynamo.Encodable, for: HashDict do
  def encode(hashdict, _) do
    %{"M" => Dynamo.Encodable.Map.do_encode(hashdict)}
  end
end

defimpl Dynamo.Encodable, for: Any do
  ...
  def deriving(module, _struct, options) do
    ...
    quote do
      defimpl Dynamo.Encodable, for: unquote(module) do
        def encode(struct, options) do
          Dynamo.Encodable.Map.encode(unquote(extractor), options)
        end
      end
    end
  end

  def encode(_, _), do: raise("Dynamo.Encodable does not fallback to any")
end

defimpl Dynamo.Encodable, for: Map do
  defmacro __deriving__(module, struct, options) do
    Dynamo.Encodable.Any.deriving(module, struct, options)
  end
  ...
  def do_encode(map) do
    Enum.reduce(map, %{}, fn
      {k, v}, map when is_binary(k) ->
        Map.put(map, k, Dynamo.Encodable.encode(v, []))

      {k, v}, map ->
        key = String.Chars.to_string(k)
        Map.put(map, key, Dynamo.Encodable.encode(v, []))
    end)
  end
end

defimpl Dynamo.Encodable, for: BitString do
  def encode(val, _) do
...
defimpl Dynamo.Encodable, for: List do
...
defimpl Dynamo.Encodable, for: MapSet do
